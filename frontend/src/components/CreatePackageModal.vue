...
import { pallets as palletsApi, packages as _packagesApi } from '../api'
...
// Pallet listesini çek (sadece MARKET tipindekiler)
const fetchPallets = async () => {
  palletsLoading.value = true
  try {
    const res    = await palletsApi.list({ 
      producer__org_code: auth.user?.orgCode, 
      current_holder__org_code: auth.user?.orgCode,
      status: 'IN_PRODUCTION' 
    })
    const data   = res.data
    pallets.value = Array.isArray(data) ? data : (data.results ?? [])
  } catch {
    error.value = 'Pallet listesi yüklenemedi.'
  } finally {
...
const handleCreate = async () => {
  ...
  error.value = null
  try {
    const _package = await _packagesApi.create({
      pallet: selectedPallet.value, 
      laying_date: form.value.layingDate,
      expiry_date: form.value.expiryDate
    })
    emit('created', _package.data)
  } catch (err) {
    ...
    >
      ...
      <div class="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <div class="flex justify-between items-center p-6 pb-0">
          <div>
            <h2 class="text-xl font-black text-stone-800">New _Packages</h2>
            <p class="text-stone-400 text-sm mt-0.5">Register a _package in the supply chain</p>
          </div>
          <button
            @click="$emit('close')"
        ...
        <div class="p-6 space-y-4">
          ...
          <select
            ...
            :class="selectedPallet ? 'border-stone-200 text-stone-800' : 'border-stone-200 text-stone-400'"
          >
          ...
          <p v-if="!palletsLoading && pallets.length === 0" class="text-xs text-amber-600 mt-1.5 ml-1">
            ⚠️ No pallets registered in the system yet.
          </p>
        </div>
        <!--  -->
        <!-- Dates Grid -->
